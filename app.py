...
def _run_apply_job(job_id, fixes_data):
    ...
    for filepath, file_fixes in fixes_by_file.items():
        ...
        prompt = (
            ...
            f"CRITICAL — Block ALL direct URL access to stages (strictly enforced):\n"
            f"- Add 'from flask import session' to the imports at the top of the file if not already present.\n"
            f"- Add 'app.secret_key = os.urandom(24)' immediately after the app = Flask(...) line if secret_key is not already set.\n"
            f"- In the /stage1 GET route: set session['stage1_done'] = True when the page is served.\n"
            f"- In the /stage2 GET route: check if session.get('stage1_done') is True. If NOT, immediately return a 403 HTML response: '<h2>Access Denied</h2><p>You must complete Stage 1 first.</p><a href=\"/stage1\">Go to Stage 1</a>'. Do NOT render the stage template.\n"
            f"- In the /stage3 GET route: check if session.get('stage2_done') is True. If NOT, return the same 403 response directing to /stage2.\n"
            f"- In the /stage4 GET route: check if session.get('stage3_done') is True. If NOT, return 403 directing to /stage3.\n"
            f"- In the /stage5 GET route: check if session.get('stage4_done') is True. If NOT, return 403 directing to /stage4.\n"
