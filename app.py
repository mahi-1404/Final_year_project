...
def _run_apply_job(job_id, fixes_data):
    ...
    for filepath, file_fixes in fixes_by_file.items():
        ...
        prompt = (
            ...
            f"CRITICAL — Stage connection removal and patched response:\n"
            ...
            f"- Replace any such progression with a clear HTML response that says: 'This vulnerability has been patched. The attack no longer works.'\n"
            f"- The patched response must also inform the user that the CTF now starts from Stage 2, not Stage 1.\n"
            f"- This applies to ALL stages being fixed in this file.\n"
            f"- For Stage 1: do NOT touch the Stage 1 HTML template at all. The 'Proceed to Stage 2' button must continue to work normally — Stage 1 is reconnaissance only and has no vulnerability to patch.\n"
            f"- For Stage 2 (XSS/SSTI): after applying the fix, when the attacker submits a malicious payload, do NOT redirect to Stage 3. Instead return a response that says: 'Cross-Site Scripting attack has been secured. This vulnerability is now patched.'\n"
            f"- For Stage 3 (XXE): after applying the fix, do NOT redirect or link to Stage 4. Return: 'XML External Entity attack has been secured. This vulnerability is now patched.'\n"
            f"- For Stage 4 (Insecure Deserialization): after applying the fix, do NOT redirect to Stage 5. Return: 'Insecure Deserialization attack has been secured. This vulnerability is now patched.'\n"
            f"- For Stage 5 (SSRF): after applying the fix, do NOT show the CTF flag or system compromised page. Return: 'Server-Side Request Forgery attack has been secured. This vulnerability is now patched.'\n\n"
            f"CRITICAL — Block direct URL access to stages:\n"
            f"- Users must ONLY be able to reach a stage by clicking the UI button from the previous stage. Direct URL navigation (e.g. typing /stage2 in the browser) must be blocked.\n"
            f"- Implement this using Flask session checks: when the Stage 1 button is clicked and /stage2 is loaded, set a session flag (e.g. session['stage1_done'] = True). Each subsequent stage route must check that the previous stage's session flag is set before rendering the page.\n"
            f"- If a user tries to access /stage2 directly without the session flag, redirect them back to /stage1 with a message: 'Access denied. You must complete the previous stage first.'\n"
            f"- Apply the same session gate for all stages: /stage2 requires stage1_done, /stage3 requires stage2_done, /stage4 requires stage3_done, /stage5 requires stage4_done.\n"
            f"- Set the session flag for each stage inside the stage's GET route handler when it is accessed legitimately.\n"
            f"- Make sure Flask secret_key is set if not already (app.secret_key = os.urandom(24) or use an existing one).\n\n"
